...
public class MemberDao {
	...
	public Member selectMember(Connection conn, String memberId) throws Exception {
		...
		try {
			...
			if (rset.next()) {
				selectMember = new Member(rset.getInt("MEMBER_NO"), rset.getString("MEMBER_ID"),
						rset.getString("MEMBER_NM"), rset.getString("MEMBER_PHONE"), rset.getString("MEMBER_EMAIL"),
						rset.getString("MEMBER_ADDR"), rset.getString("MEMBER_INTEREST"));
	...
	public int checkPwd(Connection conn, Member loginMember) throws Exception {
		...
		try {
			...
			if (rset.next()) {
				// result = rset.getInt("COUNT(*)"); 또는
				result = rset.getInt(1); // 컬럼순서를 쓸수있다.
			}
