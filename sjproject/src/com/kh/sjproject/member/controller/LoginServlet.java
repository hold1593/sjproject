...
// 매개변수에 작성된 url로  <servlet-mapping이 작성이 됨.
...
public class LoginServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			//**  page,request,session(browser),application(server)  **
			...
			if(loginMember != null) {
				...
				// 초단위로 작성 --> 10분 뒤  session 만료
				
				// 로그인이 성공한 경우
				session.setAttribute("loginMember", loginMember); // 브라우저가 꺼지지않는이상 어디서든지 사용가능(세션)
				...
				// C4) 쿠키가 사용 될 수 있는 유효한 디렉토리(경로) 설정
			...
			// referer : 웹 브라우저가 방문한 사이트 흔적을 저장하고 있음
			...
			// ** sendRedirect 와 forward 의 차이점 ** 
			// forward는 URL이 그대로이기때문에 request,response를 덮어쓸수있음
			// 
