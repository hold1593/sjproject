...
public class NoticeDao {
	...
	public List<Notice> selectList(Connection conn) throws Exception {
		...
		try {
			...
			while (rset.next()) {
				notice = new Notice(rset.getInt("NOTICE_NO"), rset.getString("NOTICE_TITLE"),
						rset.getString("NOTICE_WRITER"), rset.getInt("NOTICE_COUNT"), rset.getDate("NOTICE_MODIFY_DT")
	...
	public Notice selectNotice(Connection conn, int no) throws Exception {
		...
		try {
			...
			if (rset.next()) {
				notice = new Notice(no, rset.getString("NOTICE_TITLE"), rset.getString("NOTICE_CONTENT"),
						rset.getString("NOTICE_WRITER"), rset.getInt("NOTICE_COUNT"), rset.getDate("NOTICE_MODIFY_DT"));
			}
		} finally {
			close(rset);
