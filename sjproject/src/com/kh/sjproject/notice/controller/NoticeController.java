...
// 클라이언트의 요청 중 /notice 디렉토리 하위로 요청시
...
public class NoticeController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		// System.out.println("ccontextpath: "+ contextPath);

		String command = uri.substring((contextPath + "/notice").length()); // -> /list 가 남게됨
		...
		// 공지사항 상세조회
			...
			int no = Integer.parseInt(request.getParameter("no")); // 스트링으로 받아옴 -> int로 받아오려면 Interger.parseInt로 타입맞춰주기
		...
		// 공지사항 등록 Controller
			...
			try {
				...
				// 2) DB 저장 결과에 따라 트랜잭션 처리 후
				// 3) DB 저장 성공시 : "공지사항이 등록되었습니다"
				if (result > 0) {
					msg = "공지사항이 등록되었습니다";
